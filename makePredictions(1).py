...
def make_pred_multilabel(dataloader, model, UNCERTAINTY="zeros", alias="zeros", epoch=0, save_as_csv=False):
    """
    ...
    if save_as_csv:
        pred_df.to_csv(os.path.join("results", alias, "preds{0}.csv".format(epoch)), index=False)
        auc_df.to_csv(os.path.join("results", alias, "aucs{0}.csv".format(epoch)), index=False)
